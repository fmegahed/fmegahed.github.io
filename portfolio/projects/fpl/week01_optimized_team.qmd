...
For simplicity, I will be reading the data from the [FPL Review Site](https://fplreview.com/massive-data-planner23/). The data was downloaded on August 15, 2024 at 6:22 PM Eastern U.S. time using the download data (CSV) from the bottom of the page. Note that this data is available for subscribers only (at \$4/month). 
...
fpl_review = readr::read_csv("data/fplreview_1723760553.csv") |>
...
5. For the next three game weeks, I want to have the following four players on my bench. With the exception of Fabianski, these are starters with low (or minimal) positional cost:  
    ...
    - **Smith Rowe**, I believe he will be the best 5.0 MID in the game based on the preseason he had. He will be in my starting 11 in some game weeks and will be the first player off the bench in most game weeks.
...
The **objective** is to maximize the total expected points, including doubled points for the selected captain. This is **subject to** the following constraints (so that we do not only pick the best players from the game):  
...
$$
\text{Maximize } \sum_{i=1}^{n} \text{expected\_points}_i \cdot (x_i + y_i)
$$
...
bench_players = fpl_review |>
  dplyr::filter(web_name %in% c("Fabianski", "Barco", "Faes", "Winks")) |>
  dplyr::pull(player_index)
...
for (j in 1:20) {
  ...
  model = ompr::MIPModel() |>
    ...
    ompr::set_objective(
        ...
        ompr::sum_expr(df$expected_points[i] * y[i], i = 1:n), # double captain points
        "max"
...
DT::datatable(
    solutions[[1]], filter = 'top', rownames = FALSE,
    options = list(pageLength = 15, autoWidth = TRUE, scrollX = TRUE)
) |>
  DT::formatRound(columns=c('expected_points'), digits=1)
```
...
res = purrr::map_chr(1:length(solutions), function(j) {  
  ...
  knitr::knit_child(text = c(
    ...
    "```{r}",
    "#| echo: false",
    "DT::datatable(solutions[[j]], filter = 'top', rownames = FALSE,",
    "  options = list(pageLength = 15, autoWidth = TRUE, scrollX = TRUE)) |> DT::formatRound(columns=c('expected_points'), digits=1)",
    "```",
    ""
