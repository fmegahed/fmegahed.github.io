...
format:
  revealjs:
    ...
    footer: "[JSM 2025](https://ww3.aievolution.com/JSMAnnual2025/Events/viewEv?ev=1861) | Nashville, TN | Presentation based on [Megahed et al. (2025)](https://arxiv.org/abs/2501.12596)"
...
gray = resized_gray.convert("L")
gray_array = np.array(resized_gray)
height, width = gray_array.shape

random.seed(2025)

# Sample 50 random, non-border pixel coordinates
valid_coords = [(x, y) for x in range(10, width - 10) for y in range(10, height - 10)]
sampled_coords = random.sample(valid_coords, 20)

# Create figure (match pixel size)
fig, ax = plt.subplots(figsize=(2.5, 2.5), dpi=100)

# Display grayscale image
ax.imshow(gray_array, cmap='gray', vmin=0, vmax=255)

# Draw 1x1 pixel borders
...
# Annotate 50 sampled pixel values
for x, y in sampled_coords:
    val = gray_array[y, x]
    ax.text(x, y, str(val), color="#c3142d", fontsize=6, ha='center', va='center')

# Clean appearance
ax.set_xticks([])
ax.set_yticks([])
ax.set_xlim(-0.5, width - 0.5)
ax.set_ylim(height - 0.5, -0.5)  # Flip y-axis for correct orientation
ax.set_aspect('equal')
ax.axis('off')

# Save with no extra space
plt.subplots_adjust(left=0, right=1, top=1, bottom=0)
plt.savefig("figs/microstructure_matrix_overlay_250_sampled.png",
...
# Load the images
img1 = magick::image_read("figs/microstructure_original_250.png")
img2 = magick::image_read("figs/microstructure_grayscale_250.png")
img3 = magick::image_read("figs/microstructure_matrix_overlay_250_sampled.png")

# Combine and animate: 10 seconds per frame (i.e., 1000 ms * 10)
animated_gif = magick::image_animate(c(img1, img2, img3), delay = 800)

# Save the gif
magick::image_write(animated_gif, path = "figs/microstructure_animation.gif")
```
...
::: {.notes}
Embeddings:
Both images and text are encoded into a shared, lower-dimensional embedding space using separate image and text encoders.

Similarity Calculation:
The cosine similarity (or dot product) is computed between every image embedding and every text embedding in the batch. This results in a similarity matrix where the diagonal elements represent the similarities of the correct, matching image-text pairs, and the off-diagonal elements represent the similarities of incorrect, non-matching pairs.

Loss Calculation:
The symmetric cross-entropy loss is applied to this similarity matrix.
  - For the correct pairs (diagonal elements), the loss encourages high similarity scores.
  - For the incorrect pairs (off-diagonal elements), the loss encourages low similarity scores.

Temperature Parameter:
A learned temperature parameter is incorporated into the similarity calculation, which influences the sharpness of the probability distribution over the positive and negative pairs, helping the model learn more effectively.
...
- It also offers a **simpler and more efficient implementation**, requiring just 9.1 minutes of runtime (loading the 78 images, few-shot learning, and test classification).
